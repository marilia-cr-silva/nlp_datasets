...
df_test_aux = pd.DataFrame(
    [dataset['test']['text'], dataset['test']['label']]).T
df_validation_aux = pd.DataFrame(
    [dataset['validation']['text'], dataset['validation']['label']]).T
df_test = pd.concat([df_validation_aux, df_test_aux])
df_train = pd.DataFrame(
    [dataset['train']['text'], dataset['train']['label']]).T
...
# %%
df_content = pd.read_pickle('ed_02.pkl')
...
df_train.to_csv("ed_02_multi_train.csv", sep=";", index=False)
df_test.to_csv("ed_02_multi_test.csv", sep=";", index=False)
...
for i in tqdm(range(len(unique_classes))):
    for j in range(i+1, len(unique_classes)):
        ...
        df_aux.to_csv(f"ed_02_bin_train_{i}_{j}.csv", sep=";", index=False)

        # test
        df_aux = df_test.loc[(df_test["label"] == unique_classes[i]) | (
            df_test["label"] == unique_classes[j])]
        df_aux.to_csv(f"ed_02_bin_test_{i}_{j}.csv", sep=";", index=False)
...
explained_df.to_csv('ed_02_explained.csv', sep=";", index=False)
