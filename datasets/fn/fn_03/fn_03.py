...
df_train.to_csv(f"fn_03_multi_train.csv",sep=";",index=False)
df_test.to_csv(f"fn_03_multi_test.csv",sep=";",index=False)
...
for i in tqdm(range(len(unique_classes))):
    for j in range(i+1,len(unique_classes)):
        ...
        list_csv.append([f"fn_03_bin_train_{i}_{j}.csv",f"{unique_classes[i],unique_classes[j]}"])
        # test
        ...
        list_csv.append([f"fn_03_bin_test_{i}_{j}.csv",f"{unique_classes[i],unique_classes[j]}"])

df_list_csv = pd.DataFrame(list_csv,columns=["file_name","classes"])
df_list_csv.to_csv("fn_03_bin_explained.csv",sep=";",index=False)
