...
df_test_aux = pd.DataFrame(
    [dataset["test"]["statement"], dataset["test"]["label"]]).T
df_validation_aux = pd.DataFrame(
    [dataset["validation"]["statement"], dataset["validation"]["label"]]).T
df_test = pd.concat([df_validation_aux, df_test_aux])
df_test.columns = ["text", "label"]
df_train = pd.DataFrame(
    [dataset["train"]["statement"], dataset["train"]["label"]]).T
...
df_train.to_csv("fn_01_multi_train.csv", sep=";", index=False)
df_test.to_csv("fn_01_multi_test.csv", sep=";", index=False)
