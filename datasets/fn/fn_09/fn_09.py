...
os.chdir('/mnt/c/Users/Acer/Documents/Corpora/downloaded/FND07')

# %% creating dataframes

df_test_aux = pd.read_csv('FND07_test_with_text.csv',sep=';')
df_validation_aux = pd.read_csv('FND07_dev_with_text.csv',sep=';')
df_test = pd.concat([df_validation_aux,df_test_aux])
df_train = pd.read_csv('FND07_train_with_text.csv',sep=';')
...
for i in tqdm(range(len(unique_classes))):
    for j in range(i+1,len(unique_classes)):
        ...
        df_aux.to_csv(f"fn_09_bin_train_{i}_{j}.csv",sep=";",index=False)
        list_csv.append([f"fn_09_bin_train_{i}_{j}.csv",f"{unique_classes[i],unique_classes[j]}"])
        # test
        df_aux = df_test.loc[(df_test["label"] == unique_classes[i]) | (df_test["label"] == unique_classes[j])]
        df_aux.to_csv(f"fn_09_bin_test_{i}_{j}.csv",sep=";",index=False)
        list_csv.append([f"fn_09_bin_test_{i}_{j}.csv",f"{unique_classes[i],unique_classes[j]}"])

df_list_csv = pd.DataFrame(list_csv,columns=["file_name","classes"])
df_list_csv.to_csv("fn_09_bin_explained.csv",sep=";",index=False)
