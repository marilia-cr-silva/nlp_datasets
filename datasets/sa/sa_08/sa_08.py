...
# %% train set
df_train = pd.DataFrame([dataset['train']['review'],dataset['train']['star']]).T
...
df_train.to_csv('SA08_App_Reviews.csv',sep=';',index=False)

# %% creating binary dataframes
os.chdir('/mnt/c/Users/Acer/Documents/Corpora/Binary/Sentiment_Analysis/App_Reviews')
...
for i in tqdm(range(len(unique_classes))):
    for j in range(i+1,len(unique_classes)):
        if i != j:
            ...
            df_aux.to_csv(f'SA08_App_Reviews_{i+1}_{j+1}.csv',sep=';',index=False)
            list_csv.append([f'SA08_App_Reviews_{i+1}_{j+1}.csv',f'{unique_classes[i],unique_classes[j]}'])
...
df_list_csv.to_csv('SA08_App_Reviews_Binary_explained.csv',sep=';',index=False)