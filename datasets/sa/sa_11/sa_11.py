...
df_train.to_csv(f"sa_11_multi_train.csv",sep=";",index=False)
df_test.to_csv(f"sa_11_multi_test.csv",sep=";",index=False)
...
for i in tqdm(range(len(unique_classes))):
    for j in range(i+1,len(unique_classes)):
        ...
        df_aux.to_csv(f"sa_11_bin_train_{i}_{j}.csv",sep=";",index=False)

        # test
        df_aux = df_test.loc[(df_test["label"] == unique_classes[i]) | (df_test["label"] == unique_classes[j])]
        df_aux.to_csv(f"sa_11_bin_test_{i}_{j}.csv",sep=";",index=False)
...
explained_df.to_csv('sa_11_explained.csv', sep = ";", index=False)