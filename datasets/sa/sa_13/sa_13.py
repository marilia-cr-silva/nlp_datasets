...
# %% creating dataframes
df_test_aux = pd.DataFrame([dataset['test']['review_body'],
                           dataset['test']['stars'], dataset['test']['language']]).T
df_validation_aux = pd.DataFrame([dataset['validation']['review_body'],
                                 dataset['validation']['stars'], dataset['validation']['language']]).T
...
df_train = pd.DataFrame([dataset['train']['review_body'],
                        dataset['train']['stars'], dataset['train']['language']]).T
...
df_train.to_csv(f"sa_13_multi_train.csv", sep=";", index=False)
df_test.to_csv(f"sa_13_multi_test.csv", sep=";", index=False)
...
for i in tqdm(range(len(unique_classes))):
    for j in range(i+1, len(unique_classes)):
        ...
        df_aux.to_csv(f"sa_13_bin_train_{i}_{j}.csv", sep=";", index=False)

        # test
        df_aux = df_test.loc[(df_test["label"] == unique_classes[i]) | (
            df_test["label"] == unique_classes[j])]
        df_aux.to_csv(f"sa_13_bin_test_{i}_{j}.csv", sep=";", index=False)
...
explained_df.to_csv('sa_13_explained.csv', sep=";", index=False)
