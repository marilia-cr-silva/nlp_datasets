...
# %% creating dataframes
df_test_aux = pd.DataFrame([dataset['test_fixed']['DOCUMENT'],dataset['test_fixed']['TRUE_SENTIMENT']]).T
df_validation_aux = pd.DataFrame([dataset['validation']['DOCUMENT'],dataset['validation']['TRUE_SENTIMENT']]).T
df_test = pd.concat([df_validation_aux,df_test_aux])
df_test.columns = ['text','label']
df_train = pd.DataFrame([dataset['train']['DOCUMENT'],dataset['train']['TRUE_SENTIMENT']]).T
...
df_train.to_csv(f'SA16_PerSenT_Multi_train.csv',sep=';',index=False)
df_test.to_csv(f'SA16_PerSenT_Multi_test.csv',sep=';',index=False)

# %% creating binary dataframes
os.chdir('/mnt/c/Users/Acer/Documents/Corpora/Binary/Sentiment_Analysis/PerSenT')
...
for i in tqdm(range(len(unique_classes))):
    for j in range(i+1,len(unique_classes)):
        if i != j:
            ...
            df_aux.to_csv(f'SA16_PerSenT_Binary_train_{i}_{j}.csv',sep=';',index=False)
            list_csv.append([f'SA16_PerSenT_Binary_train_{i}_{j}.csv',f'{unique_classes[i],unique_classes[j]}'])
            # test
            df_aux = df_test.loc[(df_test['label'] == unique_classes[i]) | (df_test['label'] == unique_classes[j])]
            df_aux.to_csv(f'SA16_PerSenT_Binary_test_{i}_{j}.csv',sep=';',index=False)
            list_csv.append([f'SA16_PerSenT_Binary_test_{i}_{j}.csv',f'{unique_classes[i],unique_classes[j]}'])
...
df_list_csv.to_csv('SA16_PerSenT_Binary_explained.csv',sep=';',index=False)