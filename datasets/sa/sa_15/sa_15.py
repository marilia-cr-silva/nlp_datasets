...
# %% creating dataframes
df_test = pd.DataFrame([dataset['validation']['text'],dataset['validation']['label']]).T
df_test.columns = ['text','label']
df_train = pd.DataFrame([dataset['train']['text'],dataset['train']['label']]).T
...
df_train.to_csv(f'SA15_HopeEDI_Multi_train.csv',sep=';',index=False)
df_test.to_csv(f'SA15_HopeEDI_Multi_test.csv',sep=';',index=False)

# %% creating binary dataframes
os.chdir('/mnt/c/Users/Acer/Documents/Corpora/Binary/Sentiment_Analysis/HopeEDI')
...
for i in tqdm(range(len(unique_classes))):
    for j in range(i+1,len(unique_classes)):
        if i != j:
            ...
            df_aux.to_csv(f'SA15_HopeEDI_Binary_train_{i}_{j}.csv',sep=';',index=False)
            list_csv.append([f'SA15_HopeEDI_Binary_train_{i}_{j}.csv',f'{unique_classes[i],unique_classes[j]}'])
            # test
            df_aux = df_test.loc[(df_test['label'] == unique_classes[i]) | (df_test['label'] == unique_classes[j])]
            df_aux.to_csv(f'SA15_HopeEDI_Binary_test_{i}_{j}.csv',sep=';',index=False)
            list_csv.append([f'SA15_HopeEDI_Binary_test_{i}_{j}.csv',f'{unique_classes[i],unique_classes[j]}'])
...
df_list_csv.to_csv('SA15_HopeEDI_Binary_explained.csv',sep=';',index=False)