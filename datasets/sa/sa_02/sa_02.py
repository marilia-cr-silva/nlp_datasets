...
df_train_xml = pd.read_xml("sa_02_text_train.xml")
df_train_label = pd.read_csv("sa_02_label_train.emotions.gold",sep=" ",header=None,
...
# %%
df_test_xml = pd.read_xml("sa_02_text_test.xml")
df_test_label = pd.read_csv("sa_02_label_test.emotions.gold",sep=" ",header=None,
                        names = ["id","sent_01","sent_02",
                        "sent_03","sent_04","sent_05",
                        "sent_06"])
...
df_train.to_csv(f"sa_02_multi_train.csv",sep=";",index=False)
df_test.to_csv(f"sa_02_multi_test.csv",sep=";",index=False)
...
for i in tqdm(range(len(unique_classes))):
    for j in range(i+1,len(unique_classes)):
        ...
        df_aux.to_csv(f"sa_02_bin_train_{i}_{j}.csv",sep=";",index=False)

        # test
        df_aux = df_test.loc[(df_test["label"] == unique_classes[i]) | (df_test["label"] == unique_classes[j])]
        df_aux.to_csv(f"sa_02_bin_test_{i}_{j}.csv",sep=";",index=False)
...
explained_df.to_csv('sa_02_explained.csv', sep = ";", index=False)
