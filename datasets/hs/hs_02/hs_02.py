...
def noise_mitigation(aux):
    ...
    string = re.sub('\s\#\s|\@user\s?|Says\s|\!+\sRT\s|\s?RT\s','',string)
...
os.chdir('/mnt/c/Users/Acer/Documents/Corpora/Multiclass/Hate_Speech_Detection')
df_train.to_csv(f'HS02_Multi_train.csv',sep=';',index=False)
...
for i in tqdm(range(len(unique_classes))):
    for j in range(i+1,len(unique_classes)):
        ...
        df_aux.to_csv(f'HS02_Binary_train_{i}_{j}.csv',sep=';',index=False)
        list_csv.append([f'HS02_Binary_train_{i}_{j}.csv',f'{unique_classes[i],unique_classes[j]}'])

os.chdir('/mnt/c/Users/Acer/Documents/Corpora/Binary/Hate_Speech_Detection/Explained')
df_list_csv = pd.DataFrame(list_csv,columns=['file_name','classes'])
df_list_csv.to_csv('HS02_Binary_explained.csv',sep=';',index=False)