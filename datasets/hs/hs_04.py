...
# %% changing directory
os.chdir('/mnt/c/Users/Marília/Documents/Corpora/downloaded/HS04')
...
df_train.to_csv(f'HS04_Binary_train.csv',sep=';',index=False)
df_test.to_csv(f'HS04_Binary_test.csv',sep=';',index=False)

# %%
string = 'November 2011 21 October 2007  Jul 27, 2011 UTC 08/05/07 Please do not add biased links, which you did here: walmart. If you continue, you will face numerous problems'
string = re.sub("\d{2}\:\d{2}"
                "|\,?\s?(\d{1,2})?\s?\S{3,10}\s\d{4}\s?(UTC)?"
                "|\S{3,10}\s\d{1,2}\,\s\d{4}\s?(UTC)?"
                "|\d{2}\/\d{2}\/\d{2,4}",'',string) # time, dates
